...
google_cloud_api_key=cfg.google_cloud_api_key
...
#parcelapi = aftership.APIv4(cfg.parcelApi)
...
def gmusicselect(phrase):
    ...
    if 'album'.lower() in phrase:
        ...
        f.close()
        say("Riproduco l'album "+albumstr+" su Google Music")
        play_album(album)
    ...
    if 'artist'.lower() in phrase:
        ...
        artist=req[idx:]
        artist=artist.replace("'}", "",1)
        artist = artist.replace('artista','',1)
        if 'su'.lower() in req:
            artist = artist.replace('su','',1)
        ...
        f.close()
        say("Riproduco artista "+artiststr+" su Google Music")
        play_artist(artist)
...
def pushmessage(title,body):
    pb = Pushbullet(cfg.pushbulletKey)
...
def getrecipe(item):
    appid=cfg.edamamAppId
    appkey=cfg.edamamAppKey
